...
// real art, some locked card shows a silhouette, a duplicate shows one float
...
test("shows a duplicate badge and opens the detail popup with one float per copy", async ({
  page,
}) => {
  await loginAsOrfeu(page);
  await page.goto("/catalog");
  ...
  await expect(dialog.getByText(/^float /)).toHaveCount(count);
});
...
test("opens the popup for an unowned card without a float list", async ({ page }) => {
  await loginAsOrfeu(page);
  await page.goto("/catalog");
  ...
  await expect(dialog.getByText(/^float /)).toHaveCount(0);
});
